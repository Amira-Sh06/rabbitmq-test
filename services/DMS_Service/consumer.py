...
from common.models import ClientCreatedMessage, AcknowledgementMessage
...
class DmsConsumer:
    def __init__(self, rmq_client: RabbitMQClient, db_client: DBClient):
        ...
        self.queue_name = "dms_inbox"
        self.routing_key = "to_dms"
        self.logger = setup_logging("DMS.Consumer")
    ...
    def on_message_callback(self, ch, method, properties, body: bytes):
        try:
            ...
            if isinstance(message_obj, ClientCreatedMessage):
                self.logger.info(f"Registering new client from 1C: {message_obj.client_id} - {message_obj.client_name}")
                ...
                self.logger.info(f"Client {message_obj.client_id} registered in DB.")
                ...
                self.logger.warning(f"Unhandled message type for DMS: {action_type}. Message: {message_data}")
